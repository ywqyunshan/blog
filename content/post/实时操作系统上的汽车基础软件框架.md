...
![vcos系统架构](/static/img/vcos系统架构.png)
...
* 调度Event事件，映射Runnable到Task执行。
    ![RTE_SR](/static/img/RTE_SR.png)

### 2.2 通信模块
* 总体架构
自下而上分四层为：网络设备驱动层（net_dev）、网络协议层（net_protocols）、PDU路由层（pdu_router）和应用层（SWC）。

![网络架构](/static/img/网络管理.png)
* NM (网络管理)
研究中...
